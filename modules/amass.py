...
def run_amass(target: str, output_dir: str) -> str:
    """
    Run amass tool on the target.
    ...
    try:
        log_file = os.path.join(output_dir, "Logs", "amass.txt")
...
class AmassView(QWidget):
    ...
    def _build_ui(self):
        ...
        self.stop_button.setToolTip("Stop Amass execution")
    ...
    def _process_next_target(self):
        ...
        if not self.targets_queue:
            ...
            self._notify("Amass scan completed")
            return
        ...
        self.log_file = os.path.join(base_dir, "Logs", "amass.txt")
        os.makedirs(os.path.dirname(self.log_file), exist_ok=True)
        self.all_output = []
        self.last_base_dir = base_dir

        # Create and start worker thread
        self.worker = ProcessWorker(["amass", "enum", "-d", target])
    ...
    def _on_process_finished(self):
        ...
        if self.targets_queue:
            ...
            if hasattr(self, 'last_base_dir'):
                self._notify(f"Amass results saved under:\n{os.path.dirname(self.last_base_dir)}")
    ...
    def stop_scan(self):
        ...
        self.run_button.setEnabled(True)
        self.stop_button.setEnabled(False)
        self._notify("Amass scan stopped")
