...
class NmapView(BaseToolView):
    ...
    def update_command(self):
        ...
        if not target:
            target = "<target>"
        self.command_input.setText(f"nmap -sS {target}")

    def run_scan(self):
        raw_input = self.target_input.get_target()
        if not raw_input:
            self._notify("Please enter a target IP, range, or domain.")
            return
    ...
    def _process_next_target(self):
        if not self.targets_queue:
            ...
            self._notify("Nmap scan finished.")
            return

        target = self.targets_queue.pop(0)
        self._info(f"Running Nmap for: {target}")
        self._section(f"NMAP: {target}")

        base_dir = create_target_dirs(target, self.group_name)
        self.log_file = os.path.join(base_dir, "Nmap", "nmap_scan.txt")
        os.makedirs(os.path.dirname(self.log_file), exist_ok=True)

        command = self.command_input.text().split() + ["-oN", self.log_file]
...
class NmapTool(ToolBase):
    ...
    def get_widget(self, main_window):
        return NmapView(self.name, self.category, main_window=main_window)
